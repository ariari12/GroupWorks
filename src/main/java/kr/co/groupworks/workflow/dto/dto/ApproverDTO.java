...
public class ApproverDTO {
    @Schema(description = "결재자 번호", defaultValue = "1")
    // 1, 결재자 pk
    private long id;
    ...
    @NotNull
    @Schema(description = "결재 순서(1~5)", defaultValue = "1")
    // 3, 결재 순서
    private int sequenceNum;

    @NotNull
    @Schema(description = "결재자 구분(1:결재자, 2:협조자, 3:참조자)",  defaultValue = "1")
    // 4, 1:결재자, 2:협조자, 3:참조자 구분
    private int approverType;
    @NotNull
    @Schema(description = "결재 사원번호", defaultValue = "1")
    // 5, 결재자 사원 fk
    private long employeeId;
    @Schema(description = "결재자 이메일", defaultValue = "exampl@example.com")
    // 6, 결재자 이메일
    private String approverEmail;
    @Schema(description = "결재자 연락처", defaultValue = "010-1234-1234")
    // 7, 결재자 연락처
    private String approverPhone;
    @NotEmpty @NotNull
    @Schema(description = "결재자 명", defaultValue = "홍길동")
    // 8, 결재자 명
    private String approverName;
    @Schema(description = "결재자 직급", defaultValue = "사원")
    // 9, 결재자 직급
    private String approverRank;
    @Schema(description = "결재자 소속부서", defaultValue = "전산팀")
    // 10, 결재자 소속
    private String department;

    @Schema(description = "결재 방식 구분(1:선결, 2:후결, 3:대결, 4:전결, 5:반려)", defaultValue = "1")
    // 11, 결재 방식 1:선결, 2:후결, 3:대결, 4:전결, 5:반려
    private int approvalMethod;
    @Schema(description = "결재 방식 구분(반려사유/협의안/코멘트)", defaultValue = "승인합니다.")
    // 12, 반려사유/협의안/코멘트
    private String comment;
    @Schema(description = "결재 승인 일자", defaultValue = "1999-12-31T24:24")
    // 13, 결재 승인 일자
    private String approvalDate;
    @Schema(description = "결재 승인 일자(0:진행, 1:승인, 2:반려, 3:전결)", defaultValue = "1")
