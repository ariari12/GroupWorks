...
//        log.info("WorkFlowController - approval history");
    ...
    public String detail( HttpSession session, Model model, @PathVariable int workflowId) {
        ...
        WorkFlowDTO workFlow = workFlowService.getDetailWorkFlow(workflowId);
        ...
        model.addAttribute(AttributeName.CLASSIFICATIONS.getStatus(), classifications);
        model.addAttribute(AttributeName.COMMENT_LENGTH.getStatus(), commentLength);
        listMap.forEach(model::addAttribute);
