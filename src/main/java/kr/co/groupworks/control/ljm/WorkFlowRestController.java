...
public class WorkFlowRestController {
    ...
    private static final String APPROVE_CANSEL_PARAM = "/{workFlowId}/{status}";
...
//        log.info("WorkFlowRestController - request ok, workFlowDTO: {}", workFlowDTO);
    ...
    public ResponseEntity<Map<String, String>> fileSendTest(
            ...
            @RequestParam("pk") long pk, HttpSession session) {
...
//            log.info("WorkFlowRestController - approverSend, success");
    ...
    public ResponseEntity<Map<String, Object>> approval(@PathVariable long workFlowId,
                @PathVariable long employeeId, @PathVariable int approverType,
        ...
        /* TODO: 승인 처리 로직 */
        ApproverDTO approverDTO = new ApproverDTO().setWorkFlowId(workFlowId).setApprovalMethod(approvalMethod).setEmployeeId(employeeId).setApproverType(approverType).setComment(comment);
    ...
    /* Approval Cansel 결재 취소 */
    @DeleteMapping(value = SEPARATOR + WORKFLOW_URL + APPROVAL + APPROVE_CANSEL_PARAM)
    public String approvalCansel(@PathVariable long workFlowId, @PathVariable int status) {
        log.info("WorkFlowRestController - approval cansel, param: {}, {}", workFlowId, status);
        if(status == 0) { /* 결재가 진행 중인 경우만 발송 취소 가능 */
            /* 발송 취소 로직 처리 */

            return "redirect:" + SEPARATOR + WORKFLOW_URL + APPROVAL;
        }
        return "fail";
    }
